...
def main():
    ...
    frm_main.pack(padx=20, pady=20, fill=tk.BOTH, expand=1)
...
def populate_main_window(frm_main):
    ...
    Return: nothing
    """
    lbl_title = tk.Label(frm_main, text="JOURNAL PROMPT GENERATOR")

    # Create a label that displays "Age:"
    lbl_add_prompt = tk.Label(frm_main, text="Add a prompt to the pile:")

    # Create a integer entry box where the user will enter her age.
    ent_add_prompt = jour.IntEntry(frm_main, 1, 100, width=30)
    ...
    btn_add.grid(row=4, column=2, padx=3, pady=3, columnspan=2, sticky="W")
...
def add(ent_add_prompt):
    jour.add_prompt(ent_add_prompt)
